...
@section('subtitle', 'Edit informasi kamar hotel')
            ...
            <div class="p-6 text-gray-900">
                <form method="POST" action="{{ route('admin.rooms.update', $room) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $room->name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('description', $room->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="capacity" id="capacity" value="{{ old('capacity', $room->capacity) }}" min="1" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                        @error('capacity')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="price_per_night" id="price_per_night" value="{{ old('price_per_night', $room->price_per_night) }}" min="0" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    ...
                    <div class="mb-4">
                        <label for="available" class="flex items-center">
                            <input type="checkbox" name="available" id="available" value="1" {{ old('available', $room->available) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm align-middle">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="url" name="image" id="image" value="{{ old('image', $room->image) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                        @error('image')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
